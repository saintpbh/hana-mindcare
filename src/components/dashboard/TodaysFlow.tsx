                ...
                {SCHEDULE.map((item, index) => (
                    <div key={item.id} className={cn("relative flex gap-6 group", item.status === "completed" && "opacity-60")}>
                        ...
                        <div className="flex-1 relative group/card">
                            <Link
                                href={item.status === 'current' ? `/session/${item.id}` : `/patients/${item.id}`}
                                className="block"
                            >
                                <div className={cn(
                                    ...
                                    <h4 className={cn("text-lg font-bold mb-1 relative z-10", item.status === "current" ? "text-white" : "text-[var(--color-midnight-navy)]")}>
                                        {item.client} 님
                                    </h4>
                                    <div className={cn("flex items-center gap-2 text-xs relative z-10", item.status === "current" ? "text-white/60" : "text-[var(--color-midnight-navy)]/40")}>
                                    ...
                                    <div className="absolute inset-0 bg-white/90 opacity-0 group-hover/card:opacity-100 transition-opacity flex items-center justify-center gap-2 backdrop-blur-sm z-20">
                                        <button
                                            onClick={(e) => handleQuickAction(e, "summary", item.client)}
                                        ...
                                        {item.type === '온라인 상담' && (
                                            <button
                                                onClick={(e) => handleQuickAction(e, "link", item.client)}
                                                className="p-2 rounded-full bg-[var(--color-midnight-navy)]/5 text-[var(--color-midnight-navy)] hover:bg-[var(--color-midnight-navy)] hover:text-white transition-colors flex flex-col items-center gap-1 w-16 h-16 justify-center"
                                                title="링크 복사"
                                            >
                                        ...
                                        <button
                                            onClick={(e) => handleQuickAction(e, "delay", item.client)}
                                            className="p-2 rounded-full bg-[var(--color-midnight-navy)]/5 text-[var(--color-midnight-navy)] hover:bg-[var(--color-midnight-navy)] hover:text-white transition-colors flex flex-col items-center gap-1 w-16 h-16 justify-center"
                                            title="5분 지연 알림"
                                        >
